...
def ad_hoc_data(training_size, test_size, n, gap, plot_data=False):
    ...
    count = 0
    if n == 2: # number of features/ feature space dimension
        count = 100
    elif n == 3:
    ...
    z_m = np.asmatrix(s_z) # as matrix
    j_m = np.array([[1, 0], [0, 1]])
    j_m = np.asmatrix(j_m)

    h_m = np.array([[1, 1], [1, -1]]) / np.sqrt(2)
    h_2 = np.kron(h_m, h_m)  # outer product
    h_3 = np.kron(h_m, h_2)
    h_m = np.asmatrix(h_m)
    h_2 = np.asmatrix(h_2)
    h_3 = np.asmatrix(h_3)

    f_a = np.arange(2**n) # evenly spaced values within a given range

    my_array = [[0 for x in range(n)] for y in range(2 ** n)]
    # n features;  2**n number of states

    for arindex, _ in enumerate(my_array): # index in range 2**n
        temp_f = bin(f_a[arindex])[2:].zfill(n) 
        # bin: takes an integer and return its binary equivalent string
        # zfill: pad the left side of a string with n zeros
        for findex in range(n): # index in range n
    ...
    d_m = None
    if n == 2:
        # correspond to f first defined on page 2
        d_m = np.diag(parity) # diag: extract a diagonal or construct a diagonal array
    elif n == 3:
        d_m = np.diag(maj)

    # n qubit's Hibert space dimension
    # vector addition
    basis = aqua_globals.random.random((2 ** n, 2 ** n)) + \
        1j * aqua_globals.random.random((2 ** n, 2 ** n))

    # getH: returns the Hermitian conjugate (complex transpose)
    basis = np.asmatrix(basis).getH() * np.asmatrix(basis)

    # linalg: linear algebra
    [s_a, u_a] = np.linalg.eig(basis)

    # argsort: returns an array of indices of the same shape as s_a that index data 
    # along the last axis (-1) in sorted order
    # correspond to V on page 2
    idx = s_a.argsort()[::-1]
    s_a = s_a[idx]
    u_a = u_a[:, idx]

    # see page 3: binary measurement
    ...
    if n == 2:
        for n_1 in range(count):
            for n_2 in range(count):
                x_1 = steps * n_1
                x_2 = steps * n_2
                # see page 2: lower case letter, coefficients that encode the data x -> feature space
                # vector addition of the two feature state vectors
                phi = x_1 * np.kron(z_m, j_m) + x_2 * np.kron(j_m, z_m) + \
                    (np.pi - x_1) * (np.pi - x_2) * np.kron(z_m, z_m)
                # unitary operator from the feature state vectors
                u_u = scipy.linalg.expm(1j * phi)  # pylint: disable=no-member
                # upper case, feature space state vector -- feature map circuit
        ...
        # sample_a: those mapped to +1;  sample_b vice versa 
        t_r = 0
        ...
        # slicing sample_train by condition label_train == 0 or 1
        # key = 'A' or 'B'
        training_input = {key: (sample_train[label_train == k, :])[:training_size] # row from start to training_size
                          for k, key in enumerate(class_labels)}

        test_input = {key: (sample_train[label_train == k, :])[training_size:( # row from training_size to end
